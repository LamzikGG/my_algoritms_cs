...
public class Vector
{
    ...
    public double GetElement(int index)
    {
        if (index < 0 || index >= size)
            throw new IndexOutOfRangeException("Индекс выходит за пределы вектора");
        return data[index];
    }
    ...
    public Vector Normalize()
    {
        Vector rez = new Vector(size);
        double d = Norma1();
        if (d == 0) return rez; // Возвращаем нулевой вектор, если норма равна нулю
        for (int i = 0; i < size; i++)
            rez.data[i] = data[i] / d;
        return rez;
    }
